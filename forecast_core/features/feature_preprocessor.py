...
class PreProcessor:
    ...
    def _ensure_datetime(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        raise TypeError(f"{self.ts_col} must be pl.Datetime or pl.Utf8, got {dtype}")
    ...
    def ave_day_columns(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        # 平均対象の数値列を選ぶ
        candidate_cols = (
            self.require_cols if self.require_cols is not None else out.columns
        )
        numeric_cols = [
            col
        ...
        agg_exprs = [
            pl.col(col).mean().alias(f"{col}_daily_mean") for col in numeric_cols
        ]

        return out.group_by(key_cols).agg(agg_exprs).sort(key_cols)

    def convert_type(self, df: pl.DataFrame) -> pl.DataFrame:
        """文字列(Utf8)カラムを Categorical に変換"""
        return df.with_columns(pl.col(pl.Utf8).cast(pl.Categorical))
