...
class DataProcessor:
    def __init__(self, config):
        self.rawdata_path = Path(config.data_processor.rawdata_path)
        self.selected_cols = config.data_processor.selected_cols
        self.selected_country = config.data_processor.selected_country

    def _select_country(self, df) -> pd.DataFrame:
        """欲しい国だけ抽出するヘルパー関数"""
    ...
    def _convert_windspeed(self, df) -> pd.DataFrame:
    ...
    def _check_missing(self, df) -> None:
